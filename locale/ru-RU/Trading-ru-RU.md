...
ASF всегда принимает все обмены, независимо от предметов в них, присланные от пользователя с правами `Master` (или выше). This allows not only easily looting steam cards farmed by the bot instance, but also allows to easily manage Steam items that bot stashes in the inventory - including those from other games (such as CS:GO).
...
- Обмен **хороший** если наш прогресс к завершению значка увеличивается. Example: A A (before) <-> A B (стало)
- Обмен **нейтральный** если наш прогресс к завершению значка не меняется. Example: A B (before) <-> A C (стало)
- Обмен **плохой** если наш прогресс к завершению значка уменьшается. Example: A C (before) <-> A A (стало)
...
- ASF имеет ограничение до `40` уникальных ботов, которые могут быть сопоставлены в одном раунде, если раунд не был завершён ранее из-за того, что не осталось наборов карт требующих сопоставления.
