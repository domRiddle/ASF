...
В добавок к проверке того, что сборщик мусора находится в режиме для рабочей станции, есть ещё интересные **[настройки](https://github.com/dotnet/coreclr/blob/master/src/inc/clrconfigvalues.h)**, которые вы можете использовать. О самых интересных из них вы можете прочесть ниже.

### `GCHeapHardLimitPercent`

> Задаёт используемый размер кучи GC в процентах от общей памяти.

Эта настройка - "жесткий" предел памяти для процесса ASF, она указывает GC использовать только часть общего объёма памяти, а не всю её. Это может оказаться особенно полезным для различных серверных ситуаций, где вы можете выделить фиксированный процент памяти сервера для ASF, но не более того. Имейте в виду, что ограничение памяти ASF не отменит магическим образом все необходимые для работы выделения памяти, поэтому если вы установите здесь слишком низкое значение это может привести к ситуации с нехваткой памяти.
...
Вы можете активировать все эти настройки установив соответствующие переменные среды `COMPlus_`. Например, для Linux (shell):
