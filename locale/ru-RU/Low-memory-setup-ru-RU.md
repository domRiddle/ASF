...
Среда выполнения .NET позволяет вам **[настраивать сборщик мусора](https://docs.microsoft.com/dotnet/core/run-time-config/garbage-collector)** несколькими способами, эффективно настраивая процесс сборки мусора в соответствии с вашими потребностями.
...
```shell
# Не забудьте изменить настройку под себя, если планируете ее использовать.
...
.\ArchiSteamFarm.exe # Для сборки под конкретную OC
```
