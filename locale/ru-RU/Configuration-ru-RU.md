...
Further explanation on this subject is available in **[remote communication](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Remote-communication)** section. Если у вас нет причин редактировать этот параметр, рекомендуется оставить ему значение по-умолчанию.
...
В некоторых случаях, ASF также может сгенерировать верный код семейного просмотра Steam самостоятельно, хотя это требует значительных ресурсов ОС и дополнительного времени, не говоря уже о том, что успех не гарантируется, поэтому мы рекомендуем не полагаться на эту функцию и вместо этого указать верное значение в параметре `SteamParentalCode` конфигурационного файла для использования его в ASF. If ASF determines that PIN is required, and it'll be unable to generate one on its own, it'll ask you for input.
