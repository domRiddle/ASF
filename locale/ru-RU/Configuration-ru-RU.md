...
Если вы спонсор ASF, вы можете получить лицензию **[здесь](https://asf.justarchi.net/User/Status)**. Вам необходимо войти в GitHub для подтверждения вашей личности, мы запрашиваем только чтение публичной информации, являющейся вашим именем пользователя. `LicenseID` создаётся из 32 шестнадцатеричных символов, такой как `f6a0529813f74d119982eb4fe43a9a24`.
...
Обратите внимание что это параметр типа `flags`, и следовательно ему может быть присвоена любая комбинация приведенных выше значений. Ознакомьтесь с описанием типа **[flags](#json-mapping)** если вы хотите узнать больше. Если вы не включите никаких флагов, будет использоваться вариант `None`.
