...
`FarmingOrder` - параметр типа `byte` со значением по-умолчанию `​0​`. Этот параметр задаёт **предпочитаемый** порядок фарма в ASF. На данный момент вам доступны следующие варианты порядка фарма:
...
`BotName.maFile` is a special file that can be used for importing **[ASF 2FA](https://github.com/JustArchi/ArchiSteamFarm/wiki/Two-factor-authentication)**. Это не обязательный файл, и не создаётся, а только распознаётся ASF если ваш бот `BotName` ещё не имеет подключенного 2ФА ASF. Этот файл автоматически удаляется после того как 2ФА ASF успешно импортирован.
