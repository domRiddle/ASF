...
`Success` - значение типа `bool`, предоставляющее простой способ проверки результата. Это дополнение к кодам состояния HTTP, поскольку коды `2xx` считаются `true`, а всё остальное считается `false`. Пожалуйста, обратите внимание, что это свойство индицирует только **что запрос API удался**. В стандартных ответах с результатом (описанных ниже), вам следует разобрать поле `Result` для проверки, было ли конкретное действие завершено успешно (например, в этом поле может быть вывод команды).
