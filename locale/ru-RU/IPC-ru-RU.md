...
Если вам не нужен доступ из любых источников, а нужен, например, только из локальной сети, гораздо лучше будет использовать что-то вроде `192.168.0.0` вместо `*`. Измените сетевой адрес на правильный, если у вас используется другой.