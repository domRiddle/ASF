...
Для просмотра полного списка поддерживаемых .NET Core 3.0 платформ и ОС, посетите раздел "**[release notes](https://github.com/dotnet/core/blob/master/release-notes/3.0/3.0-supported-os.md)**" в репозитории .NET Core.
...
Мы рекомендуем отказаться от OpenVZ в пользу гораздо лучших решений для виртуализации, таких как KVM. Описанная здесь проблема это действительно ошибка в .NET Core, которая должна быть **[исправлена](https://github.com/dotnet/coreclr/pull/26912)** в следующем обновлении среды исполнения .NET Core (3.1, возможно и 3.0), но для этого исправления пока не назначены определённые сроки. Если у вас нет возможности перейти на лучшее решение для виртуализации - вы можете подумать о использовании ASF в варианте `generic-netf` с помощью `mono`, во всяком случае до тех пор, пока не будет выпущена новая версия среды исполнения.

Разумеется, вы также можете использовать предыдущую версию ASF (V4.0), по использование устаревших версий ASF не поддерживается нами, и вы можете столкнуться с различными проблемами, возможно даже более серьёзными.