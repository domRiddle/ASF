...
Обновление с Debian 9 Stretch (или более старого) на Debian 10 Buster обновит общесистемные настройки `libssl` по умолчанию до версии 1.1. До выпуска .NET Core 3.0 (и обновления ASF для работы с ним), вам понадобиться вручную указать среде выполнения .NET Core необходимость загрузки `libssl` версии 1.1 вместо 1.0 (которая является предпочтительной до тех пор, пока она существует).

Самый удобный способ добиться этого - задать переменную среды `CLR_OPENSSL_VERSION_OVERRIDE=1.1`, это решит проблему, без необходимости понижения общесистемных настроек `libssl`, и без необходимости полного удаления `libssl` версии 1.0. Вы можете задать переменные среды как минимум несколькими различными способами, но если вы не знаете что для вас лучший вариант - мы рекомендуем добавить её в файл `/etc/environment` (и перезагрузить машину).

В качестве альтернативы, как и в случае выше, вы можете убедиться что у вас **нет** пакета `libssl1.0.2`, например командой `apt-get purge libssl1.0.2`. Это устаревший пакет, и он отсутствует и недоступен для установки в чистом Debian 10, и единственный путь столкнуться с такой проблемой - это обновление с Debian 9 или более старого - **[dotnet/corefx #33179](https://github.com/dotnet/corefx/issues/33179)**. Однако, как минимум несколько пакетов зависит от этого пакета (например, `dotnet-runtime-2.2`, который необходим для запуска ASF в универсальном варианте), поэтому это может оказаться невозможным в вашем окружении. После выхода .NET Core 3.0 и ASF будет обновлён для использования этой версии, эта проблема больше не будет актуальна.