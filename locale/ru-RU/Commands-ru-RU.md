...
Начиная с V4.0.0.9, в ASF добавлена поддержка интерактивной консоли, которую можно включить установив параметр [**`SteamOwnerID`**](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-ru-RU#steamownerid). После этого, достаточно нажать клавишу `c` чтобы активировать командный режим, ввести свою команду и нажать ввод для подтверждения.

![Скриншот](https://i.imgur.com/bH5Gtjq.png)

Интерактивная консоль недоступна в режиме [**`Headless`**](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-ru-RU#headless).
...
![Скриншот](https://raw.githubusercontent.com/JustArchiNET/ASF-ui/master/.github/previews/commands.png)
...
| `input [Bots] <Type> <Value>`                            | `Master`        | Задаёт входные данные для указанного значения для заданных ботов, работает только в режиме `Headless` - подробное описание **[ниже](#user-content-Команда-input)**.                                                                                                                                                                  |
...
| `loot@ [Bots] <RealAppIDs>`                                    | `Master`        | Отправляет все предметы Сообщества, соответствующие `LootableTypes`, из игры с `appID` равным заданному `RealAppIDs` от заданных ботов к их `Master` заданному в `SteamUserPermissions` (с самым меньшим steamID, если их больше одного). Это команда, противоположная `loot%`.                                                      |
| `loot% [Bots] <RealAppIDs>`                                    | `Master`        | Отправляет все предметы Сообщества, соответствующие `LootableTypes`, за исключением предметов из игры с `appID` равным заданному `RealAppIDs` от заданных ботов к их `Master` заданному в `SteamUserPermissions` (с самым меньшим steamID, если их больше одного). Это команда, противоположная `loot@`.                             |
...
| `redeem^ [Bots] <Modes> <Keys>`                          | `Operator`      | Активирует указанные ключи или коды пополнения на заданных ботах, с использованием режимов `modes`, подробно описанных **[ниже](#user-content-Режимы-активации-redeem)**.                                                                                                                                                            |
| `reset [Bots]`                                                       | `Master`        | Сбрасывает статус "в игре" к статусу по умолчанию, используется для остановки ручного режима фарма заданного командой `play`.                                                                                                                                                                                                        |
...
| `transfer@ [Bots] <RealAppIDs> <TargetBot>`              | `Master`        | Отправляет все предметы сообщества Steam соответствующие `TransferableTypes` и полученные из игры с AppID равным `RealAppIDs` от заданных ботов боту `TargetBot`. Это команда, противоположная `transfer%`.                                                                                                                          |
| `transfer% [Bots] <RealAppIDs> <TargetBot>`              | `Master`        | Отправляет все предметы сообщества Steam соответствующие `TransferableTypes` за исключением предметов из игры с AppID равным `RealAppIDs` от заданных ботов боту `TargetBot`. Это команда, противоположная `transfer@`.                                                                                                              |
