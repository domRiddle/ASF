...
| `loot <Bots>`                                  | `Master`        | Отправляет все предметы, соответствующие `LootableTypes`, от заданных ботов к их `Master` заданному в `SteamUserPermissions` (с самым меньшим steamID, если их больше одного).                                                         |
