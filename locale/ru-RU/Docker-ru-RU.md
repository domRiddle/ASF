...
Важно отметить, что запуск ASF в Docker-контейнере считается **расширенной установкой**,что **не требуется** для подавляющего большинства пользователей и обычно **не дает никаких преимуществ** по сравнению с установкой без контейнеров. Если вы рассматриваете Docker как решение для запуска ASF в качестве службы, вы должны задуматься о прочтении раздела **[управления](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Management#systemd-service-for-linux)** и установке соответствующего `сервиса`, который будет **почти всегда** лучше, чем запуск ASF в контейнере Docker.
