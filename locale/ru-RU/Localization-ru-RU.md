...
- Mistake in the source string - Ошибка в исходной строке. Этот тип должен использоваться только если вы полагаете что исходная (английская) строка неверна. Quite rare, but not all of us are speaking English natively either, so feel free to use it if you have a general idea how it could be improved. В качестве альтернативы, поскольку это непосредственно связано с разработкой, вы можете использовать для этой цели наши **[выпуски GitHub](https://github.com/JustArchiNET/ArchiSteamFarm/issues/new/choose)**, если хотите.
