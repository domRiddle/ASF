...
Да, ASF даже не скачивает файлы игры, поэтому он будет работать со всеми лицензиями на вашем аккаунте Steam, независимо от платформы или технических требований. It should also work for games tied to specific region (region-locked games) even when you're not in the matching region, although we don't guarantee that (it worked last time we tried).
...
> In regard to all Subscriptions, Content and Services that are not authored by Valve, Valve does not screen such third-party content available on Steam or through other sources. Valve не несет никакой ответственности за контент сторонних производителей. Some third-party application software is capable of being used by businesses for business purposes - however, you may only acquire such software via Steam for private personal use.
...
Вы можете запускать сколько угодно копий ASF на одной машине, если у каждой копии своя директория и свои файлы конфигураций, и аккаунты используемые в одной копии не используются в других. Однако, спросите себя, зачем вы этого хотите. ASF оптимизирован для запуска десятка, или даже сотню аккаунтов одновременно, и запуск десятка ботов в разных экземплярах ASF снижает производительность, отнимает дополнительные ресурсы ОС (такие как время ЦП и память), и может приводить к проблемам синхронизации между разными экземплярами ASF, поскольку ASF вынужден разделять свои ограничители с другими копиями.
...
> Вы несете ответственность за конфиденциальность Вашего имени пользователя и пароля и за безопасность Вашей компьютерной системы. Valve is not responsible for the use of your password and Account or for all of the communication and activity on Steam that results from use of your login name and password by you, or by any person to whom you may have intentionally or by negligence disclosed your login and/or password in violation of this confidentiality provision.
...
Эта проблема практически всегда вызвана отключенной/остановленной службой Windows `Изоляция ключей CNG`, которая предоставляет базовую функциональность криптографии для ASF, без которой программа не может работать. Вы можете исправить эту проблему, запустив `services.msc` и убедившись что служба Windows `Изоляция ключей CNG` сейчас выполняется и у неё не отключен автозапуск.
...
ASF использует сборку в один файл в вариантах под конкретные ОС, из-за чего при запуске приложение сначала распаковывается (при необходимости) во временную папку `<tmp>/.net`. On Windows, this is `%TEMP%\.net` (usually `C:\Users\<YourUser>\AppData\Local\Temp\.net`), on Linux, this is `/var/tmp/.net`. Папка `.net` может не существовать по умолчанию, он создаётся при первой необходимости.

Первая проблема может быть вызвана тем, что ASF не может распаковать ASF в эту директорию, вторая проблема это повреждённые при распаковке файлы - например, если вы закрыли ASF до того, как все файлы были распакованы. Обычно самым простым решением этих проблем, как под Windows так и под Linux, будет удалить временную папку `.net`, описанную выше, и попробовать снова, обычно это должно решить проблему.

Процессу ASF необходимы права на запись в указанную выше папку. Под Windows это обычно не проблема, но под Linux вам надо убедиться, что пользователь, под которым запускается процесс ASF имеет доступ на запись в папку `/var/tmp/.net`, обычно это так и есть, но иногда от вас могут потребоваться дополнительные действия если вы используете не-стандартные разрешения или нечто подобное.

В качестве альтернативы, вы также можете задать своё значение переменной окружения `DOTNET_BUNDLE_EXTRACT_BASE_DIR`, указав желаемый путь к папке куда следует распаковывать программы. Это может быть особенно полезно если вы не хотите чтобы ASF использовал папку по умолчанию для распаковки. Имейте в виду, что к новой папке применимы те же требования по правам доступа.
