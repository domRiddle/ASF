...
Поэтому я **настоятельно рекомендую** всегда запускать максимум одну копию на один IP/интерфейс. If you have more IPs/interfaces, by all means you can run more ASF instances, with every instance using its own IP/interface or unique `WebProxy` setting. Если же нет - запуск нескольких экземпляров ASF совершенно не имеет смысла, вы не получите никакой выгоды от запуска более одного экземпляра на один IP/интерфейс. Steam не позволит вам волшебным образом запустить больше ботов только потому что вы их запускаете в другой копии ASF, и если уж на то пошло - ASF вас никак не ограничивает.
