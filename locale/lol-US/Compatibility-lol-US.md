...
4 COMPLETE LIST OV ALL SUPPORTD PLATFORMS AN OSEZ BY .NET 8.0, VISIT **[RELEASE NOTEZ](https://github.com/dotnet/core/blob/main/release-notes/8.0/supported-os.md)**.
...
ASF as a program is targeting **.NET 8.0** (`net8.0`) right now, but it may target newer platform in the future. `net8.0` is supported since 8.0.100 SDK (8.0.0 runtime), although ASF might prefer **latest runtime at the moment of compilation**, so you should ensure that you have **[latest SDK](https://dotnet.microsoft.com/download)** (or at least runtime) available for your machine. GENERIC ASF VARIANT CUD REFUSE 2 LAUNCH IF UR RUNTIME IZ OLDR THAN TEH SPECIFID MINIMUM SUPPORTD WAN DURIN COMPILASHUN.
