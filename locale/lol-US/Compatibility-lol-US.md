...
This approach has gigantic amount of advantages, as CIL is platform-independent, which is why ASF can run natively on many available OSes, especially Windows, Linux and macOS. There is not only no emulation needed, but also support for all platform-related and hardware-related optimizations, such as CPU SSE instructions. THX 2 DAT, ASF CAN ACHIEVE SUPERIOR PERFORMANCE AN OPTIMIZASHUN, WHILE STILL OFFERIN PERFIK COMPATIBILITY AN RELIABILITY.
...
- `osx-arm64` works on 64-bit ARM-based (Apple silicon) macOS OSes. DIS INCLUDEZ VERSHUN 11, AS WELL AS FUCHUR ONEZ.
- `osx-x64` works on 64-bit macOS OSes. DIS INCLUDEZ VERSHUN 10.15, AS WELL AS FUCHUR ONEZ.
