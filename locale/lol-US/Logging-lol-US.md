...
*steamID* - SteamID DECLARD AS 64-BIT LONG UNSIGND INTEGR OV TARGET STEAM USR (LIEK `SteamOwnerID`), OR TARGET `chatID` (WHEN `chatGroupID` IZ SET). REQUIRD. Defaults to `0` which disables logging target entirely.

*botName* - Name of the bot (as it's recognized by ASF, case-sensitive) that will be sending messages to `steamID` declared above. NOT REQUIRD. DEFAULTS 2 `null` WHICH WILL AUTOMATICALLY SELECT **ANY** CURRENTLY CONNECTD BOT. IZ RECOMMENDD 2 SET DIS VALUE APPROPRIATELY, AS `SteamTarget` DOEZ NOT TAEK INTO AKOWNT LOTZ DA STEAM LIMITASHUNS, SUCH AS TEH FACT DAT U MUST HAS `steamID` OV TEH TARGET ON UR FRIENDLIST. DIS VARIABLE IZ DEFIND AS [LAYOUT](https://github.com/NLog/NLog/wiki/Layouts) TYPE, THEREFORE U CAN USE SPESHUL SYNTAX IN IT, SUCH AS `${logger}` IN ORDR 2 USE TEH BOT DAT GENERATD TEH MESAGE.
