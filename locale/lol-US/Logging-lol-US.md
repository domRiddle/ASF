...
*botName* - NAYM OV TEH BOT (AS IZ RECOGNIZD BY ASF, CASE-SENSITIV) DAT WILL BE SENDIN MESAGEZ 2 `steamID` DECLARD ABOOV. NOT REQUIRD. DEFAULTS 2 `null` WHICH WILL AUTOMATICALLY SELECT **ANY** CURRENTLY CONNECTD BOT. IZ RECOMMENDD 2 SET DIS VALUE APPROPRIATELY, AS `SteamTarget` DOEZ NOT TAEK INTO AKOWNT LOTZ DA STEAM LIMITASHUNS, SUCH AS TEH FACT DAT U MUST HAS `steamID` OV TEH TARGET ON UR FRIENDLIST. DIS VARIABLE IZ DEFIND AS [LAYOUT](https://github.com/NLog/NLog/wiki/Layouts) TYPE, THEREFORE U CAN USE SPESHUL SYNTAX IN IT, SUCH AS `${logger}` IN ORDR 2 USE TEH BOT DAT GENERATD TEH MESAGE.
