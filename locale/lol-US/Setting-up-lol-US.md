...
AFTR DOWNLOAD, START FRUM EXTRACTIN TEH ZIP FILE INTO ITZ OWN FOLDR. We recommend using **[7-zip](https://www.7-zip.org)**, but all standard utilities like `unzip` from Linux/macOS should work without problems as well.
...
PLZ NOWT DAT SUM FEATUREZ, SUCH AS SENDIN COMMANDZ, REQUIRE PROPERLY SET `SteamOwnerID` GLOBAL CONFIG PROPERTY. NAO DAT U HAS ASF-UI UP AN RUNNIN, Y NOT GIV IT TRY AN SET IT FRUM TEH FRONTEND ITSELF? ULL NED 2 INPUT UNIQUE STEAM IDENTIFICATOR IN 64-BIT FORM OV UR AKOWNT. U CAN LOOK IT UP IN VARIOUS DIFFERENT WAYS, 4 EXAMPLE THRU **[STEAMID I/O](https://steamid.io)** OR **[STEAMREP](https://steamrep.com)**. TEH NUMBR URE LOOKIN 4 SHUD BE SIMILAR 2 `76561198006963719`, WHICH IZ MAH AKOWNTS ID.
...
Helper scripts (such as `ArchiSteamFarm.cmd` for Windows and `ArchiSteamFarm.sh` for Linux/macOS) are located next to `ArchiSteamFarm.dll` binary - those are included in `generic` variant only. U CAN USE THEM IF U DOAN WANTS 2 EXECUTE `dotnet` COMMAND MANUALLY. OBVIOUSLY HELPR SCRIPTS WONT WERK IF U DIDNT INSTALL .NET SDK AN U DOAN HAS `dotnet` EXECUTABLE AVAILABLE IN UR `PATH`. HELPR SCRIPTS R ENTIRELY OPSHUNAL 2 USE, U CAN ALWAYS `dotnet /path/to/ArchiSteamFarm.dll` MANUALLY.