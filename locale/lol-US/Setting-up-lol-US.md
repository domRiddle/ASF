...
ASF supports farming more than one account at a time, which is its primary function. U CAN ADD MOAR ACCOUNTS 2 ASF BY GENERATIN MOAR BOT CONFIG FILEZ, IN EGSAKTLY TEH SAME WAI AS UVE GENERATD UR FURST WAN JUS FEW MINUTEZ AGO. U NED 2 ENSURE ONLY 2 THINGS:
...
U CHANGE EXISTIN SETTINGS IN EGSAKTLY TEH SAME WAI - BY GENERATIN NEW CONFIG FILE. IF U DIDNT CLOSE R WEB CONFIG GENERATOR YET, CLICK ON "TOGGLE ADVANCD SETTINGS" AN C WUT IZ THAR 4 U 2 DISCOVR. For this tutorial we'll change `CustomGamePlayedWhileFarming` setting, which allows you to set custom name being displayed when ASF is farming, instead of showing actual game.
