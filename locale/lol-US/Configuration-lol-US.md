...
IN ORDR 2 USE ASF-UI, FIRSTLY U MUST ENABLE R **[IPC](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/IPC-lol-US)** INTERFACE ITSELF. U CAN DO DAT BY USIN R WEB-BASD CONFIGGENERATOR EXPLAIND ABOOV, GENERATIN VRY SIMPLE `ASF` CONFIG WIF ENABLD `IPC` SETTIN AN NOTHIN ELSE. ALTERNATIVELY, U CUD ALSO GENERATE SUCH SIMPLE CONFIG YOURSELF, AS `ASF.json` FILE WIF `{ "IPC": true }` JSON CONTENT INSIDE.

AFTERWARDZ, LAUNCH ASF WIF TEH ABOOV CONFIG, ENSURE DAT `IPC` INTERFACE IZ STARTD, DEN NAVIGATE 2 ASFS **[IPC ADDRES](http://localhost:1242)**. U CAN NAO DO TEH REMAININ CONFIGURASHUN OV ASF THRU ASF-UI INTERFACE.
...
`bool` TYPE WIF DEFAULT VALUE OV `true`. DIS PROPERTY DEFINEZ IF ASF IZ PERMITTD 2 IDLE GAMEZ DAT R STILL REFUNDABLE. A REFUNDABLE GAME IZ GAME DAT U BUYD IN LAST 2 WEEKZ THRU STEAM STORE AN DIDNT PULAY 4 LONGR THAN 2 HOURS YET, AS STATD ON **[STEAM REFUNDZ](https://store.steampowered.com/steam_refunds)** PAEG. BY DEFAULT WHEN DIS OPSHUN IZ SET 2 `true`, ASF IGNOREZ STEAM REFUNDZ POLICY ENTIRELY AN IDLEZ EVRYTHIN, AS MOST PEEPS WUD EXPECT. HOWEVR, U CAN CHANGE DIS OPSHUN 2 `false` IF U WANTS 2 ENSURE DAT ASF WONT IDLE ANY OV UR REFUNDABLE GAMEZ 2 SOON, ALLOWIN U 2 EVALUATE DOSE GAMEZ YOURSELF AN REFUND IF NEEDD WITHOUT WORRYIN BOUT ASF AFFECTIN PLAYTIME NEGATIVELY. PLZ NOWT DAT IF U DISABLE DIS OPSHUN DEN GAMEZ U PURCHASD FRUM STEAM STORE WONT BE IDLD BY ASF 4 UP 2 14 DAIS SINCE REDEEM DATE, WHICH WILL SHOW AS NOTHIN 2 IDLE IF UR AKOWNT DOESNT OWN ANYTHIN ELSE. IF URE UNSURE WHETHR U WANTS DIS FEACHUR ENABLD OR NOT, KEEP IT WIF DEFAULT VALUE OV `true`.
