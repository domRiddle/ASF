...
There is also idling priority queue that is accessible through `fq` **[commands](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands)**. IF IZ USD, AKSHUL FARMIN ORDR IZ SORTD FIRSTLY BY PERFORMANCE, DEN BY IDLIN KEW, AN FINALLY BY UR `FarmingOrders`.

---

### `FarmPriorityQueueOnly`

`bool` TYPE WIF DEFAULT VALUE OV `false`. This property defines if ASF should consider for automatic farming only apps that you added yourself to priority idling queue available with `fq` **[commands](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands)**. WHEN DIS OPSHUN IZ ENABLD, ASF WILL SKIP ALL `appIDs` DAT R MISIN ON TEH LIST, EFFECTIVELY ALLOWIN U 2 CHERRY-PICK GAMEZ 4 AUTOMATIC ASF FARMIN. KEEP IN MIND DAT IF U DIDNT ADD ANY GAMEZ 2 TEH KEW DEN ASF WILL ACT AS IF THAR IZ NOTHIN 2 FARM ON UR AKOWNT. IF URE UNSURE WHETHR U WANTS DIS FEACHUR ENABLD OR NOT, KEEP IT WIF DEFAULT VALUE OV `false`.
