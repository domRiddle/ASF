...
IF U WANTS 2 PROVIDE `<key>` FRUM FILE, USE `--cryptkey-file` 4 DAT PURPOSE INSTEAD, EXPLAIND BELOW.
...
`--cryptkey-file <path>` OR `--cryptkey-file=<path>` - WILL START ASF WIF CUSTOM CRYPTOGRAFIC KEY READ FRUM `<path>` FILE. DIS SERVEZ TEH SAME PURPOSE AS `--cryptkey <key>` EXPLAIND ABOOV, ONLY TEH MECHANISM DIFFERS, AS DIS PROPERTY WILL READ `<key>` FRUM PROVIDD `<path>` INSTEAD.
