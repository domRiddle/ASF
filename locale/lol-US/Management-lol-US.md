...
4 FURTHR ELABORASHUN ON *Y* WE DISCOURAGE RUNNIN ASF AS ROOT, REFR 2 **[SUPERUSR](https://superuser.com/questions/218379/why-is-it-bad-to-run-as-root)** AN OTHR RESOURCEZ. If you're still not convinced, ask yourself what would happen to your machine if ASF process executed `rm -rf /*` command right after its launch.
...
IF UD LIEK 2 UTILIZE R **[`systemd` SERVICE](#systemd-service-4-linux)** EXPLAIND ABOOV 4 MULTIPLE ASF INSTANCEZ, IZ VRY SIMPLE, JUS USE ANOTHR USR 4 R `ArchiSteamFarm@` SERVICE DECLARASHUN AN FOLLOW WIF TEH REST OV TEH STEPS.