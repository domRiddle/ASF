...
IF UD LIEK 2 UTILIZE R **[`systemd` SERVICE](#systemd-service-4-linux)** EXPLAIND ABOOV 4 MULTIPLE ASF INSTANCEZ, IZ VRY SIMPLE, JUS USE ANOTHR USR 4 R `ArchiSteamFarm@` SERVICE DECLARASHUN AN FOLLOW WIF TEH REST OV TEH STEPS. This will be equivalent of running multiple ASF instances with distinct binaries, so they can also auto-update and operate independently of each other.