...
ASF 会尽全力减少操作系统层面的与其他 ASF 实例的跨进程通信。 这包括 ASF 会读取其他实例的配置文件目录，并且共享由 `*LimiterDelay` **[全局配置属性](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration#global-config)**&#8203;配置的进程级信号量，确保运行多个 ASF 实例不会导致频率限制问题。 Windows 平台会为此使用原生的操作系统级信号量，Unix 平台会回退到 ASF 自定义的基于文件的锁机制，这些锁文件会被放在 `/tmp/ASF` 目录下。
...
ASF 在决定共享范围时会考虑到 `WebProxy` 设置，即使用不同 `WebProxy` 的 ASF 实例之间不会采用同一个限制。 实现此功能是为了让 `WebProxy` 设置不会导致过大的操作延迟，符合使用不同网络接口的预期。
