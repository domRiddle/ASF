...
每个机器人都是一个在 ASF 进程中运行的单独的 Steam 账号。 为了能够正常工作，ASF 需要至少定义**一个**机器人实例。 进程不会为机器人实例设定强制的数量限制，所以您可以运行任意数量的机器人（Steam 账户）。
