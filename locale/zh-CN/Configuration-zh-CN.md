...
| 64 | MarkTradeMessagesAsRead       | 使 ASF 自动将聊天内的交易提醒标记为已读                |
...
`MarkBotMessagesAsRead` 和 `MarkTradeMessagesAsRead` 的运作方式很相似，但它们只会将特定的消息标记为已读。 但要注意的是，Steam 在将一条消息标记为已读时，**同样**会标记此消息**之前**的所有消息，所以如果您如果不希望错过特定事件之间的消息，就不应该启用此类选项。
...
此外，您可以通过 `iq` **[命令](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Commands-zh-CN)**&#8203;访问“挂卡优先级队列”。 如果使用这个队列，则实际的挂卡顺序首先由性能决定，其次受优先挂卡队列影响，最后才是 `FarmingOrders`。
