...
La configuration de `MatchActively` est une version étendue de `SteamTradeMatcher` qui en plus de la correspondance passive offerte par cette option, inclut également la correspondance active dans laquelle le bot enverra des transactions à d'autres personnes.
