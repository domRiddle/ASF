...
<tr>
  <td><code>play <Bots> <AppIDs,GameName>` | `Maître` | Basculement vers le farming manuelle - lancez `AppIDs` sur des instances de bot, éventuellement avec ` GameName ` de façon personnalisé. Utilisez `reset` ou `resume` pour retourner. |
| `privacy <Bots> <Settings>`                                    | `Maître`          | Remplace **[Steam privacy settings](https://steamcommunity.com/my/edit/settings)** les instances de bots données par des options correctement sélectionnées, expliquées **[ci-dessous](#privacy-settings)**.                                                                                                                                                    |
