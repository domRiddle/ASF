...
| `owns <Bots> <Games>`                                          | `Opérateur`       | Checks if given bot instances already own given `games`, explained **[below](#owns-games)**.                                                                                                                                                                                                                                                                    |
...
<tr>
  <td><code>play <Bots> <AppIDs,GameName>` | `Maître` | Bsculement vers le farming manuell - lancez `AppIDs` sur des instances de bot, éventuellement avec ` GameName </ 0> de façcon personnalisé. Use <code>reset` or `resume` for returning. |
...
| `redeem <Bots> <Keys>`                                         | `Opérateur`       | Réclamer clés-Cd ou les codes de portefeuille Steam dans des instances de bot données.                                                                                                                                                                                                                                                                          |
| `redeem^ <Bots> <Modes> <Keys>`                          | `Opérateur`       | Utilise des clés-Cd ou des codes de portefeuille Steam sur des instances de bot données, en utilisant les `modes` donnés **[ci-dessous](#redeem-modes)**.                                                                                                                                                                                                       |
| `reset <Bots>`                                                       | `Maître`          | Resets the playing status back to normal, used during manual farming with `play` command.                                                                                                                                                                                                                                                                       |
...
| `transfer@ <Bots> <RealAppIDs> <TargetBot>`              | `Maître`          | Sends all `TransferableTypes` Steam community items matching given `RealAppIDs` from given bot instances to target bot instance.                                                                                                                                                                                                                                |
