...
<p>C'est une bonne idée d'avoir une traduction cohérente, même si elle pourrait potentiellement être libérée du processus de révision / vote de la communauté expliqué ci-dessus. This is mainly because incorrect translations that are not necessarily bad may get so many upvotes that it's no longer possible to suggest any better translation, even if somebody has such.</p>
...
<li>Mistake in the source string - this should be used only if you believe that original (English) string is incorrect. Quite rare, but I'm not speaking English natively either, so feel free to use it if you have a general idea how it could be improved. Alternatively, since this is strictly related to the development, you may use our <strong><a href="https://github.com/JustArchiNET/ArchiSteamFarm/issues/new/choose">GitHub issues</a></strong> for that purpose, if you'd like to.</li>
</ul>
