...
| `play <Bots> <AppIDs,GameName>`                                | `주인(Master)`           | Switches to manual farming - launches given `AppIDs` on given bot instances, optionally also with custom `GameName`. Use `reset` or `resume` for returning.                                                                            |
