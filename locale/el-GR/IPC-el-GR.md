...
ASF περιλαμβάνει τη δική του μοναδική διεπαφή IPC που μπορεί να χρησιμοποιηθεί για περαιτέρω αλληλεπίδραση με τη διαδικασία. IPC σημαίνει **[inter-process communication](https://en.wikipedia.org/wiki/Inter-process_communication)** και στον πιο απλό ορισμό αυτό είναι το "ASF web interface" με βάση **[Kestrel HTTP server](https://github.com/aspnet/KestrelHttpServer)** που μπορεί να χρησιμοποιηθεί για περαιτέρω ενσωμάτωση στη διαδικασία, τόσο ως frontend για τον τελικό χρήστη (ASF-ui), όσο και ως backend για την ενσωμάτωση τρίτων (ASF API).

IPC μπορεί να χρησιμοποιηθεί για πολλά διαφορετικά πράγματα, ανάλογα με τις ανάγκες και τις ικανότητές σας. For example, you can use it for fetching status of ASF and all bots, sending ASF commands, fetching and editing global/bot configs, adding new bots, deleting existing bots, submitting keys for **[BGR](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Background-games-redeemer)** or accessing ASF's log file. All of those actions are exposed by our API, which means that you can code your own tools and scripts that will be able to communicate with ASF and influence it during runtime. In addition to that, selected actions (such as sending commands) are also implemented by our ASF-ui which allows you to easily access them through a friendly web interface.
