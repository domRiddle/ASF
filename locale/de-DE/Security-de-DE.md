...
Dies ist die einfachste und zugleich unsicherste Methode das Passwort zu speichern, definiert als `PasswordFormat` mit einem Wert von `0`. ASF expects `SteamPassword` property to be a plain text - password being used to log in to Steam in its direct form. Es ist das am einfachsten zu bedienende und 100% kompatible mit allen Systemen, daher ist es der Standard.
