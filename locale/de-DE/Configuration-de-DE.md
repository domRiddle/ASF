...
`string` - String-Typ der jede beliebige Zeichenfolge akzeptiert, einschließlich einer leeren Zeichenfolge wie `""` und `null`. Sowohl die leere Zeichenfolge als auch der Wert `null` werden von ASF gleich behandelt, so dass es deiner Präferenz überlassen bleibt, welche davon du verwenden möchtest.
