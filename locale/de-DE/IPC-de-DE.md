...
Die zweite Möglichkeit besteht darin eine **[benutzerdefinierte Konfiguration](#benutzerdefinierte-konfiguration)** für die IPC-Schnittstelle von ASF zu spezifizieren, wo Sie https-Endpunkt aktivieren und das entsprechende Zertifikat direkt an unseren Kestrel http-Server senden können. Dieser Weg wird empfohlen, wenn Sie keinen anderen Webserver betreiben und keinen ausschließlich für ASF betreiben möchten. Andernfalls ist es viel einfacher ein befriedigendes Setup zu erreichen, indem man einen Reverse-Proxy-Mechanismus verwendet.
...
ASF enthält zusätzliche Sicherheitsmaßstäbe, die standardmäßig nur die Loopback-Schnittstelle (`localhost`, Ihre eigene Maschine) zum Zugriff auf die ASF-API erlaubt; ohne `IPCPassword` in der Konfiguration gesetzt. Dies liegt daran, dass die Verwendung von `IPCPassword` der **minimale** Sicherheitsmaßstab sein sollte, der von allen festgelegt wird, die sich entscheiden, die ASF Schnittstelle weiter zu enthüllen.
