...
ASF meldet sich über unseren integrierten Mini-Steam-Client mit den von Ihnen angegebenen Anmeldeinformationen beim Steam-Konto an. Nach erfolgreicher Anmeldung analysiert es Ihre **[Abzeichen](https://steamcommunity.com/my/badges)** um Spiele zu finden, die für das Sammeln verfügbar sind ("X Karten verbleibend"). Nachdem alle Seiten analysiert und eine endgültige Liste der verfügbaren Spiele erstellt wurde, wählt ASF den effizientesten Sammel-Algorithmus aus und startet den Prozess. Der Prozess hängt von dem gewählten **[Karten-Sammel-Algorithmus](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Performance-de-DE#Leistung)** ab, aber normalerweise besteht er aus dem Spielen des geeigneten Spiels und der periodischen (plus bei jedem Kartendrop) Überprüfung, ob das Spiel bereits vollständig gesammelt wurde - wenn ja, kann ASF mit dem nächsten Spiel fortfahren, mit dem gleichen Verfahren, bis alle Spiele vollständig im gesammelt wurden.
...
Du kannst so viele ASF-Instanzen auf einem Computer ausführen, wie du willst, vorausgesetzt, jede Instanz hat ihr eigenes Verzeichnis und ihre eigenen Konfigurationen, und das in einer Instanz verwendete Konto wird in einer anderen nicht verwendet. Allerdings solltest du dich fragen, warum du das tun willst. ASF ist dazu optimiert, mehrere dutzend Bots gleichzeitig zu verarbeiten, während mehrere Bots in separate ASF-Instanzen ausgeführt werden. Das führt allerdings auch dazu, dass die Leistung der einzelnen Instanzen sinkt, da mehr Betriebssystem-Ressourcen (beispielsweise CPU und Arbeitsspeicher) verbraucht werden und birgt die Gefahr von Synchonitäts-Problemen zwischen den verschiedenen ASF-Instanzen, da ASF seine Einschränkungen ("Limiters" ) mit den anderen Instanzen teilen muss.
