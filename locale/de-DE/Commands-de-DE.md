...
| `addlicense [Bots] <Licenses>`                                 | `Operator`      | Aktiviert angegebene `Lizenzen`, wie **[unten](#addlicense-licenses)** erklärt, in den angegebenen Bot-Instanzen (nur kostenlose Spiele).                                                                                                                                                                                                                                |
...
| `encrypt <encryptionMethod> <stringToEncrypt>`           | `Owner`         | Verschlüsselt die Zeichenkette mit der bereitgestellten kryptographischen Methode - ausführlicher erklärt weiter **[unten](#encrypt-command)**.                                                                                                                                                                                                                          |
...
| `hash <hashingMethod> <stringToHash>`                    | `Owner`         | Erzeugt ein Hash der Zeichenkette mit der bereitgestellten kryptographischen Methode - ausführlicher erklärt weiter **[unten](#hash-command)**.                                                                                                                                                                                                                          |
...
Mit dem Encrypt-Befehl kannst du beliebige Zeichenketten mit den ASF-Verschlüsselungsmethoden verschlüsseln. `<encryptionMethod>` muss eine der Verschlüsselungsmethoden sein, welche im Abschnitt **[Sicherheit](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Security)** angegeben und erklärt ist. Dieser Befehl ist nützlich, wenn du im Voraus verschlüsselte Details erzeugen möchtest, z. B. um zu vermeiden, dass du zuerst dein `PlainText`-Passwort in die Konfiguration eingibst und dann den Befehl `Passwort` verwendest. Wir empfehlen, diesen Befehl über sichere Kanäle zu verwenden (ASF-Konsole oder IPC-Schnittstelle, für die es auch einen dedizierten API-Endpunkt gibt), da sonst sensible Details von verschiedenen Dritten protokolliert werden könnten (z. B. Chat-Nachrichten, die von Steam-Servern protokolliert werden).

* * *

## `hash` Befehl

Mit dem Hash-Befehl kannst du Hashes beliebiger Zeichenketten mittels der ASF-Hashing-Methoden erzeugen. `<hashingMethod>` muss eine der Hashing-Methoden sein, welche im Abschnitt **[Sicherheit](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Security)** angegeben und erklärt ist. Wir empfehlen, diesen Befehl über sichere Kanäle zu verwenden (ASF-Konsole oder IPC-Schnittstelle, für die es auch einen dedizierten API-Endpunkt gibt), da sonst sensible Details von verschiedenen Dritten protokolliert werden könnten (z. B. Chat-Nachrichten, die von Steam-Servern protokolliert werden).
