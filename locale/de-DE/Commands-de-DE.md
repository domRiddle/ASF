...
| `loot@ <Bots> <RealAppIDs>`                                    | `Master`        | Sendet alle `LootableTypes` Steam-Community-Gegenstände, die mit den angegebenen `RealAppIDs` der angegebenen Bot-Instanzen übereinstimmen, an `Master` welcher in den `SteamUserPermissions` (mit der niedrigsten SteamID, wenn mehr als eine) definiert ist. |
...
| `transfer^ <Bots> <AppID> <ContextID> <TargetBot>` | `Master`        | Sendet alle Steam-Gegenstände von der angegebenen `AppID` in `ContextID` der angegebenen Bot-Instanzen an den Ziel-Bot.                                                                                                                                        |
