...
ASF will reject trade offer, regardless of content, from any (non-master) user that is blacklisted from trading module. Blacklist is stored in standard `BotName.db` database, and can be managed via `bl`, `bladd` and `blrm` **[commands](Commands)**. This should work as an alternative to standard user block offered by Steam - use with caution.
...
It's nice to note that `AcceptDonations` doesn't require **[ASF 2FA](Escrow)**, as there is no confirmation needed if we're not losing any items.
