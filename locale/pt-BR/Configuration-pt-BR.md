...
Por favor, note que devido a constantes problemas e mudanças no Steam, **nós não damos nenhuma garantia de que esta função vai funcionar corretamente**, portanto, é inteiramente possível que ela **não funcione de modo algum**. Não aceitamos **nenhum** relatório de bugs, nem pedidos de suporte para essa opção. Ele é oferecido com absolutamente nenhuma garantia, você está usando por seu próprio risco.

---

### `BotBehaviour`

Tipo `byte flags` com o valor padrão `0`. Esta propriedade define o comportamento do bot do ASF durante vários eventos e é definida como abaixo:
