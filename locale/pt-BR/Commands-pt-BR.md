...
| `encrypt <encryptionMethod> <stringToEncrypt>`           | `Owner`             | Criptografa a string usando o mecanismo de criptografia padrão - melhor explicado **[abaixo](#comando-encrypt)**.                                                                                                                                                                                                                                                        |
...
O comando encrypt permite que você criptografe sequências arbitrárias usando os mecanismos de criptografia do ASF. `<encryptionMethod>` must be one of the encryption mechanisms specified and explained in **[security](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Security)** section. Esse comando é útil caso você queira gerar detalhes criptografados antecipadamente, por exemplo, para evitar colocar a sua senha em `PlainText` na configuração e depois usar o comando `password`. Recomendamos usar esse comando através de canais seguros (como o console do ASF ou a interface IPC, que também tem uma API endpoint dedicada a isso), uma vez que detalhes sensíveis podem, de outra forma, serem registrados por vários terceiros (como as mensagens de bate-papo registradas pelos servidores Steam).

* * *

## `hash` command

Hash command allows you to generated hashes of arbitrary strings using ASF's hashing mechanisms. `<hashingMethod>` must be one of the hashing mechanisms specified and explained in **[security](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Security)** section. Recomendamos usar esse comando através de canais seguros (como o console do ASF ou a interface IPC, que também tem uma API endpoint dedicada a isso), uma vez que detalhes sensíveis podem, de outra forma, serem registrados por vários terceiros (como as mensagens de bate-papo registradas pelos servidores Steam).
