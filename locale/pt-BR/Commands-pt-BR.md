...
`input MySteamGuardBot SteamGuard ABCDE` -> Definimos a entrada `SteamGuard` do bot `MySteamGuardBot` para `ABCDE`. Of course, `ABCDE` in this case is auth code that we got on our e-mail.
