...
Todos os comandos abaixo são afetados pelo **[parâmetro de configuração global](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Configuration-pt-BR#configura%C3%A7%C3%A3o-global)** `CommandPrefix`, que é, por padrão, `!`. Isto significa que para executar, por exemplo, o comando `status`, você deve escrever `!status` (ou o `CommandPrefix` configurado de sua escolha).
...
| `loot@ <Bots> <RealAppIDs>`                                    | `Master`            | Envia todos itens da comunidade Steam que se enquadram como `LootableTypes`, e cujo `RealAppIDs` corresponda ao indicado, dos bots indicados para o usuário definido como `Master` em `SteamUserPermissions` (para o de steamID mais baixo caso haja mais de um). |
...
| `loot& <Bots>`                                                   | `Master`            | Alterna o envio de itens dos bots indicados entre habilitado/desabilitado.                                                                                                                                                                                        |
| `nickname <Bots> <Nickname>`                                   | `Master`            | Muda o apelido Steam dos bots indicados para o informado em `nickname`.                                                                                                                                                                                           |
| `owns <Bots> <AppIDsOrGameNames>`                              | `Operator`          | Verifica se os bots indicados já possuem os `appIDs` e/ou `gameNames` (nome dos jogos) citados (pode ser apenas parte do nome do jogo). Também pode ser `*` para mostrar todos os jogos disponíveis.                                                              |
...
| `play <Bots> <AppIDs,GameName>`                                | `Master`            | Muda para o modo de coleta manual: abre o jogo indicado pela `AppIds` no bot indicado, opcionalmente pode-se usar um nome customizado `GameName`. Use o comando `resume` para retornar a coleta automática.                                                       |
...
| `redeem <Bots> <Keys>`                                         | `Operator`          | Ativa as `cd-keys` informadas nas contas bots indicadas.                                                                                                                                                                                                          |
| `redeem^ <Bots> <Modes> <Keys>`                          | `Operator`          | Ativa as `cd-keys` informadas nas contas bots indicadas, utilizando os `modes` (métodos) explicados **[abaixo](#métodos-redeem)**.                                                                                                                                |
