...
This approach has gigantic amount of advantages, as CIL is platform-independent, which is why ASF can run natively on many available OSes, especially Windows, Linux and macOS. There is not only no emulation needed, but also support for all platform-related and hardware-related optimizations, such as CPU SSE instructions. Graças a isso, o ASF pode alcançar desempenho e otimização superiores, enquanto continua a oferecer compatibilidade e confiabilidade perfeitos.
...
- `osx-arm64` works on 64-bit ARM-based (Apple silicon) macOS OSes. Isso inclui a versão 11, bem como futuras versões.
- `osx-x64` works on 64-bit macOS OSes. Isso inclui a versão 10.15, bem como futuras versões.
