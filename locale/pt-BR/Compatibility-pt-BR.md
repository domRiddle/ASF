...
Isto também significa que o ASF **não tem nenhum requisito específico quanto ao Sistema Operacional**, porque ele exige um **tempo de execução** funcional no Sistema Operacional e não o Sistema Operacional em si. Enquanto o tempo de execução estiver rodando o código do ASF corretamente, não importa se o Sistema Operacional é Windows, Linux, OS X, BSD, Sony Playstation 4, Nintendo Wii ou a sua torradeira: uma vez que rode o **[.NET](https://dotnet.microsoft.com/download/dotnet)**, roda o **[ASF](https://github.com/JustArchiNET/ArchiSteamFarm/releases/latest)** (na variante genérica).
