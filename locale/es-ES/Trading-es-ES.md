...
El ajuste `MatchActively` es la versión activa de `SteamTradeMatcher` que incluye emparejamiento interactivo en el cual el bot enviará intercambios a otras personas. Puede funcionar solo, o junto con el ajuste `SteamTradeMatcher`.

Para usar esa opción, tienes que cumplir ciertos requisitos. Tienes que ser elegible para nuestra **[lista ASF STM](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Remote-communication-es-es#lista-p%C3%BAblica-asf-stm)**, con requisitos un poco relajados. Como mínimo debes tener una cuenta **[deslimitada](https://support.steampowered.com/kb_article.php?ref=3330-IAGK-7663)**, **[ASF 2FA](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/Two-factor-authentication-es-es#asf-2fa)** activo y por lo menos un tipo válido en `MatchableTypes`, tal como los cromos.
