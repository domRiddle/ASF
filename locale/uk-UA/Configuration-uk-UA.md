...
параметр типу `bool` зі значенням за замовчуванням `false`. Цей параметр визначає, чи буде **[IPC](https://github.com/JustArchiNET/ArchiSteamFarm/wiki/IPC-uk-UA)** сервер ASF запускатися при старті процесу. IPC дає можливість взаємодії між процесами шляхом запуску локального серверу HTTP. Якщо ви не плануєте користуватися IPC-сервером ASF то вам немає сенсу активувати цю опцію.
