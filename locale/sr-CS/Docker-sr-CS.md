...
Zbog toga što `poslednja` verzija dolazi sa mogućnosti za auto-ažuriranje, ona sadrži onsovni OS sa OS-specifičnom ASF verzijom, suprotno svim ostalim tagovima koji sadrže OS sa .NET Core runtime i `opštom` ASF verzijom. This is because newer (updated) ASF version might also require newer runtime than the one the image could possibly be built with, which would otherwise require image to be re-built from scratch, nullifying the planned use-case.
...
## Argumentni na komandnoj liniji
