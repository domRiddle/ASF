...
Native dependencies are generated as part of OS-specific builds, as there is no .NET Core runtime available on the host and ASF is running through its own .NET Core runtime that is bundled as part of OS-specific build. In order to minimize the build size, ASF trims its native dependencies to include only the code that can be possibly reached within the program, which effectively cuts the unused parts of the runtime. Ovo vam može izazvati potencijalan problem sa vašin pluginom, ako se zadesite u situaciji gdje vaš plugin zavisi od neke .NET Core karakteristike koja nije upotrebljena u ASF-u, i koju OS-build te verzije ne može pokrenuti, često ako dodate `System.MissingMethodException` ili `System.Reflection.ReflectionTypeLoadException` može pomoći.
