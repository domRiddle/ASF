...
上述大多數套件應早已安裝於您的系統中。 The minimal installation of Debian stable required only `libicu72`.
...
In order to use ASF-ui, you need to have `IPC` enabled, which is the default option. 啟動ASF之後，您應該能夠看到它自動成功開啟IPC介面的訊息：
