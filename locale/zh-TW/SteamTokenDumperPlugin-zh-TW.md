...
在開啟 ASF 時，外掛程式會以標準 ASF 日誌的形式通知是否成功啟用。 您也可以在設定檔生成器網頁工具中啟用外掛程式。
