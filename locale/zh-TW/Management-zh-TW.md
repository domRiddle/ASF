...
ASF含有邏輯，驗證自身是否以系統管理員（&#8203;`root`&#8203;）身分執行。 若環境設定正確，ASF程序的任何操作都&#8203;**不**&#8203;需要執行於Root，因此使用Root執行都被視為&#8203;**糟糕的方法**&#8203;。 這代表在Windows上，ASF永遠都不應該使用「以系統管理員身分執行」設定來執行；而在Unix上，ASF應要擁有專用的使用者帳號，或在桌面環境中使用您自己的帳號。
