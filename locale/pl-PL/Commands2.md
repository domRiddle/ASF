...
`loot^ <AppID> <ContextID>` | `Master` | Sends all items from given `AppID` of `ContextID` of current bot instance to `Master` user defined in its `SteamUserPermissions` (with lowest steamID if more than one).
