...
If you're confused about above statement and you don't know better, check which `dll` libraries are included in `ASF-generic.zip` package and ensure that your plugin includes only those that are not part of that zip archive yet. This will be only `YourNamespace.YourPluginName.dll` for the most simple plugins.