...
- If you're already using SteamDesktopAuthenticator, WinAuth or any other third-party app that allows you to extract 2FA details with ease, just **[duplicating](#import)** those to ASF.
- If you're using official app and you don't mind resetting your 2FA credentials, the best way is to disable 2FA, then use **[joint authenticator](#joint-authenticator)** which will allow you to use official app and ASF 2FA, this method doesn't require root or advanced knowledge, barely following instructions.
...
- If you're not using 2FA yet and don't care, you can use ASF 2FA with **[standalone authenticator](#standalone-authenticator)**, third-party app **[duplicating](#imported)** to ASF, or **[joint authenticator](#joint-authenticator)** with official app.
...
3. Execute the `2fainit [Bot]` command, replacing `[Bot]` with your bot's name.
...
4. Execute the `2fafinalize [Bot] <ActivationCode>` command, replacing `[Bot]` with your bot's name and `<ActivationCode>` with the code you've received through SMS in the previous step.
...
4. Ignore the SMS that you received after the previous step.
5. Install the Steam mobile app if it's not installed yet, and open it. Navigate to the Steam Guard tab and add a new authenticator by following the app's instructions.
6. After your authenticator in the mobile app is added and working, return to ASF. You now need to tell ASF that finalization is done with the help of one of the two commands below:
 ...
 - If you don't want to check if codes are the same and are sure that everything is fine, you can instead use the command `2fafinalizedforce [Bot]`, replacing `[Bot]` with your bot's name. ASF will assume that the authenticator was added correctly and proceed with importing your newly created authenticator.
