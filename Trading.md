...
- Each matching is composed of "rounds", ASF has upper hard limit of rounds that can happen in a single matching session.
...
- ASF has upper limit of unique bots it can match in a single round, if not cancelled before due to running out of sets to match.
