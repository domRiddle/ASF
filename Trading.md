...
In addition to that, ASF will also accept all ```!loot``` trades being sent across bots, unless ```DontAcceptBotTrades``` is specified in ```TradingPreferences``` (together with lack of ```AcceptDonations```). In short, default ```TradingPreferences``` of ```None``` will cause ASF to automatically accept trades from ```SteamMasterID``` (explained above), as well as all donation trades from other bots that are taking part in ASF process. If you want to disable donation trades from other bots, then you must disable ```AcceptDonations``` (default) and enable ```DontAcceptBotTrades``` in your ```TradingPreferences```.
