...
`Host` accepts a variety of values, including `*` that works the same as `0.0.0.0`. Be extremely careful when you use `Host` values that allow remote access, such as `*`. Doing so will enable access to ASF's IPC interface from other machines, which might pose a security risk. We strongly recommend to use `IPCPassword` and your own firewall at a minimum in this case.
