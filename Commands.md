...
`!loot^ <Bots>` | `Master` | Sends all items from given `AppID` and `ContextID` of given bot instances to `Master` user defined in its `SteamUserPermissions` (with lowest steamID if more than one).
