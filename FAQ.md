...
**A:** No, binaries that are provided in official **[releases](https://github.com/JustArchi/ArchiSteamFarm/releases/latest)** are OS-independent, and work with both .NET framework (Windows), as well as Mono (Linux, OS X and other OSes). Of course, you **can** **[compile and use ASF from source](https://github.com/JustArchi/ArchiSteamFarm/wiki/Compilation)** if you have a reason to do so, but it's absolutely not required, neither recommended if you just want to launch ASF.
