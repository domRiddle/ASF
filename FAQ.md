...
Another potential solution is not using login keys entirely with `UseLoginKeys` config property set to false, but you should do that only if ASF has fully automated way to make initial login. Right now this is possible only with using valid `SteamPassword` and **[ASF 2FA](https://github.com/JustArchi/ArchiSteamFarm/wiki/Escrow)**. This way we don't need to rely on login keys at all, since we have required login credentials (password and 2FA key) available on each login.
