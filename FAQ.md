...
Of course, if your API key got leaked, we recommend to secure your account and revoke the old one immediately, which is completely supported by ASF - it'll generate a new one on the next run. It's not possible for ASF to entirely skip using API keys though, exclusively due to the fact that if we could access the information we need through different channels, there would be no requirement of using API key already.
