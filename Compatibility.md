...
As the time goes with more platforms supported by .NET Core and less compatibility between .NET Framework and .NET Core, this package will be entirely replaced. Please refrain from using it if you can use any .NET Core package instead, as it's missing a lot of functionality and compatibility compared to .NET Core versions, and it'll miss only even more in the future.
