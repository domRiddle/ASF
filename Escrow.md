...
If you don't want or don't need to go through WinAuth, then simply copy ```/data/data/com.valvesoftware.android.steam.community/files/Steamguard-XXX``` where XXX is your ```SteamID``` of the account you want to add (if more than one, because if you have only one then this will be the only file). This file is protected and you won't be able to access ```/data/data``` without root access. You should then put this file as ```Bot.maFile``` in ASF config directory, where ```Bot``` is name of the bot you want to add 2FA to. After this step, launch ASF - it should notice the ```.maFile``` and import it.
