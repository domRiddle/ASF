...
b) If your account has API key set, is using ASF 2FA, and includes ```SteamTradeMatcher``` in ```TradingPreferences```, then ASF will periodically communicate with our **[server](https://asf.justarchi.net)**. Actual data consists of unique ASF ID (generated by ASF), and following account-related information:
