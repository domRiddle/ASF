...
b) If your account is unrestricted, using ASF 2FA, has public inventory with Steam cards in it and includes ```SteamTradeMatcher``` in ```TradingPreferences```, then ASF will periodically communicate with our **[server](https://asf.justarchi.net)**. Actual data consists of unique ASF ID (generated by ASF), and following account-related information:
