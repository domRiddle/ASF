...
Since Steam group in no way identifies the account of using ASF program or not, we do not consider your membership as sensitive detail. It's only a good indicator for us what userbase ASF consists of.

b) If your account is unrestricted, using ASF 2FA, has public inventory with at least 100 `MatchableTypes` items in it and includes `SteamTradeMatcher` in `TradingPreferences`, then ASF will periodically communicate with our **[server](https://asf.justarchi.net)**. Actual data consists of unique ASF ID (generated by ASF), and following account-related information:
