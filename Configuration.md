...
Enabling both `Forwarding` and `Distributing` will add forwarding feature on top of distributing one, which effectively makes ASF trying to redeem one key on all bots first before moving on to the next one. This effectively means that if you're redeeming 5 keys for 5 of your bots and bot 2 and 4 own the game already, then with this option your last 2 keys will stay unused, as opposed to key 2nd and 4th that would be left unused when distributing alone. Typically you want to use this option only when you want `Forwarding`, but with altered behaviour of switching the bot on key being used, instead of always going in-order with every key (which would be `Forwarding` alone).
