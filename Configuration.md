...
```Paused``` - ```bool``` type with default value of ```false```. This property defines initial state of ```CardsFarmer``` module. With default value of ```false```, bot will automatically start farming when it's started, either because of ```StartOnLaunch``` or ```!start``` command. Switching this property to ```true``` should be done only if you want to manually ```!resume``` automatic farming process, for example because you want to use ```!play``` all the time and never use automatic ```CardsFarmer``` module. If you're unsure whether you want this feature enabled or not, keep it with default value of ```false```.

```StartOnLaunch``` - ```bool``` type with default value of ```true```. This property defines if bot should automatically start when it's instance is created by ASF (so on ASF launch). Usually you want to keep it at ```true```, but you may also want to keep bot ```Enabled```, yet ```Stopped```. If you decide to change this property to ```false```, you'll need to ```!start``` given bot instance manually on every ASF startup. Unless you have a reason to edit this property, you should keep it at default.
