...
```HackIgnoreMachineID``` - ```bool``` type with default value of ```false```. This property acts as a workaround for broken GenerateMachineID() function in SK2. If ASF is "stuck" right after ```Connected to Steam!``` and ```Logging in...```, then you may need to enable this option. This option is a hack, and **will be removed as soon as GenerateMachineID() bug will be fixed**. Refer to https://github.com/JustArchi/ArchiSteamFarm/issues/154 and https://github.com/SteamRE/SteamKit/issues/254 for more info. Unless you have a **strong** reason to edit this property, you should keep it at default.
