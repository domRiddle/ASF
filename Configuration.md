...
```ForceHttp``` - ```bool``` type with default value of ```false```. By default ASF is trying to make use of secure ```https``` protocol whenever possible, which should be the preferred way of using it. However, in some rare situations, you may want to switch from ```https``` back to ```http```, which has lower overhead and is more compatible. You can do that by switching ```ForceHttp``` to ```true```. Using this option does not guarantee all requests being sent by ASF to be ```http```, as some services ASF is using (such as GitHub api) are supporting ```https``` only. If you're not debugging your network traffic, it is highly recommended to keep using secure and encrypted ```https```, instead of insecure and unencrypted ```http```. Unless you have a **strong** reason to edit this property, you should keep it at default.
...
```LogToFile``` - ```bool``` type with default value of ```true```. This property defines if ASF should maintain ```log.txt``` with the log of it's latest run. As explained above, log file is useful for you as it allows you to analyze what ASF is doing, and log file is crucial if you decide to report some ASF bug - it doesn't log any sensitive information. On the other hand, if you have many bot instances, it's possible that log file will grow big, and instead of letting ASF to log "everything" to it's log, you might want to redirect standard output (```stdout```) through the filters of your choice. Unless you have a **strong** reason to edit this property, you should keep it at default.
...
```SteamMasterID``` - ```ulong``` type with default value of ```0```. This property defines steamID in 64-bit form of bot master, which is you. If you're configuring ```primary``` account, then you don't need to put anything here and you can leave it with default value of ```0```. When you're configuring an ```alt``` account, this property defines steamID which is allowed to issue core functions, such as the one that is allowed to issue **[commands](https://github.com/JustArchi/ArchiSteamFarm/wiki/Commands)**. ASF will also accept friend requests, chat invites and all trades sent by ```SteamMasterID```. You can check your steamID in 64-bit form e.g. **[here](https://steamrep.com/)**, by logging in and navigating to your profile. The example steamID we're looking for looks like this: ```76561198006963719```.
