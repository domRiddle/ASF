...
However, there is one catch with `Invisible` mode - it doesn't go well with primary accounts. This is because **any** Steam session that is currently online **exposes** the status, even if ASF itself does not. This is the current limitation/bug of the Steam network that isn't possible to be fixed on ASF side, so if you want to use `Invisible` mode you will also need to ensure that **all** other sessions to the same account use `Invisible` mode as well. Most of the time you want to show as `Online` to your friends, hiding only ASF activity, so `Invisible` mode in this case will not work at all, unless you set your own Steam client to use `Invisible` as well. Hopefully this limitation/bug will be solved in the future, making `Invisible` session independent of other ones (which is how it should work to begin with).
