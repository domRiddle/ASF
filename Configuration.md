...
```Paused``` - ```bool``` type with default value of ```false```. This property defines if bot should automatically start farming when it's started, either because of ```StartOnLaunch``` or ```!start``` command. Setting this property to ```true``` should be done only if you want to manually ```!resume``` automatic farming process, for example because you want to farm manually via ```!play``` all the time and never use automatic ```CardsFarmer``` module. If you're unsure whether you want this feature enabled or not, keep it with default value of ```false```.
