...
In short, if your `steamID` is taking part in the same ASF process, then the minimum logging level of your `SteamTarget` should be `Info` or `Debug` (if you're not running ASF in `Debug` mode).